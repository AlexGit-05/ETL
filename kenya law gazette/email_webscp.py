...
# Define the connection string to the Postgres database using SQLAlchemy
legal = f"postgresql://{username}:{password}@localhost:5432/legal"
...
def scrape_kenya_gazettes():
    ...
    try:
        ...
        if soup.find("td", class_="cell-date").text.strip() == '':
...
######################################################################################################################
def highlight_and_save_pdf():
    """
    Highlights specific terms in the PDF and saves it.
    Returns the path to the highlighted PDF.
    """
    input_pdf_path = "downloads/download.pdf"
    output_pdf_path = "downloads/highlighted_energy_doc.pdf"
    ...
    try:
        ...
        logging.error(f"Error highlighting PDF: {str(e)}")
        return None

def send_mail(FROM, TO, SUBJECT, SERVER, PASSWORD, date_obj=None, data=None):
    """
    Sends an email with the recent notice details and highlighted PDF if applicable.
    ...
    # Convert URLs to clickable links with custom text
    ...
    try:
        ...
        # Check if there are any EPRA/Energy Act related entries
        if data['EPRA/Energy Act'].notna().any():
            # Highlight and attach PDF
            highlighted_pdf_path = highlight_and_save_pdf()
            if highlighted_pdf_path and os.path.exists(highlighted_pdf_path):
                body += """
                <p><strong>Note:</strong> A highlighted PDF with relevant Energy Act and EPRA references is attached.</p>
                """
                ...
                with open(highlighted_pdf_path, 'rb') as f:
                    ...
                    pdf_attachment.add_header('Content-Disposition', 'attachment', 
                                           filename='highlighted_energy_doc.pdf')
                    message.attach(pdf_attachment)
                logging.info("Highlighted PDF attached to email")
        ...
        # Clean up the highlighted PDF after sending
        if os.path.exists('downloads/highlighted_energy_doc.pdf'):
            os.remove('downloads/highlighted_energy_doc.pdf')
            logging.info("Cleaned up highlighted PDF file")
...
def upload_notice(engine):
    ...
    try:
        ...
        table = Table('gazett_notice_db', metadata, autoload_with=engine)
        ...
        if Date is not None:
            ...
            if not result:
                ...
                if not result:
                    # Re-extract the notice information
                    Date, df = scrape_kenya_gazettes()
                    df.to_sql(name='gazett_notice_db', con=engine, if_exists='append', index=False)
